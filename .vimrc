...
function! g:InstallPackages(package_mangager, install_command, query_command, packages)
    ...
    for i in filter([a:package_mangager, a:install_command], '!type(v:val) == 2')
        echoerr '[ERROR] Bad type of '.i
        return
    endfor

    if !executable(a:package_mangager)
        echoerr '[ERROR] Package manger '.a:package_mangager.' not executable!'
        return
    endif

    if !type(a:packages) == 3
        echoerr '[ERROR] Bad type of packages expected List<String> got '.a:packages
        return
    endif

    if len(a:packages) == 0
        echoerr '[ERROR] no packages passed, got an empty list!'
        return
    endif

    if !executable('bash')
        echoerr '[ERROR] bash not executable!'
        return
    endif
