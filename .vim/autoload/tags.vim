...
fu! tags#lib(age_min, force, lib_path, ...)
    ...
    let l:subdir = empty(&filetype) ? expand('%:e') : &filetype
    ...
    if a:force || utils#is_stale(l:tag_file, a:age_min) 
        let l:cmd = 'ctags -f '.l:tag_file.' $(find '.join([a:lib_path] + a:000, ' ').' -readable -type f -name '.string('*.'.l:ext).')'
        echom 'generating library indecies'
        echom l:cmd
        call system(l:cmd)
        if v:shell_error
            echoerr 'an error occurred '.v:shell_error
        endif
    endif
...
fu! tags#project(anchors, force)
    ...
    if !empty(l:ext) && (a:force || utils#is_stale(l:tag_file, 10))
        ...
        let l:cmd = "ctags -f ".l:tag_file." $(find ".l:root." -type f -name '*.".l:ext."')"
        echom l:cmd
        call system(l:cmd)
        if v:shell_error
            echoerr 'an error occurred '.v:shell_error
